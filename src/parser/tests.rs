...
fn test_successful_credential_parsing() {
    ...
    let credential = Credential::new(account.clone());
    let result = credential.get_credentials(data.to_string());

    expect_that!(result, ok(eq(&ParseResult {
...
fn test_account_not_found() {
    ...
    expect_that!(result, err(matches_pattern!(ParsingError::AccountNotFound(_)))); 
}
...
fn test_malformed_credentials() {
    ...
    expect_that!(result, ok(matches_pattern!(ParseResult{
        username: eq(""),
        password: eq("")
    })))
}
