...
use crate::{decryptor, encryptor::encrypt_file, paths::get_encrypted_file_path};
...
fn test_add_to_encrypted_file() {
    ...
    new_credential
        .add_to_encrypted_file(password)
...
fn test_add_to_encrypted_file_fails_if_no_encrypted_file() {
    ...
    expect_that!(
        result,
        err(matches_pattern!(FileError::DoesnotExist(
            contains_substring(ENCRYPTED_FILENAME)
        )))
    );
}
